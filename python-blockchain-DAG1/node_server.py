...
@app.route('/changedifficulty', methods=['POST'])
def changedifficulty():
    difficulty_ = request.get_json()
    print(difficulty_['difficulty'])
    print("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
    if difficulty_['difficulty'] > blockchain.difficulty:
        blockchain.difficulty = difficulty_['difficulty']
        return "change difficulty" , 200

def send_difficulty():
    content = senddifficulty(blockchain.difficulty)
    for peer in peers:
        url = "{}changedifficulty".format(peer)
        headers = {'Content-Type': "application/json"}
        requests.post(url,
                      data=json.dumps(content.__dict__, sort_keys=True),
                      headers=headers)
...
def verify_and_add_block():
    ...
    proof = block_data['hash']
    # f = open("error.txt",mode="a")
    # f.write("input block = {} \r\nneed hash = {} \r\n".format(block.previous_hash,blockchain.chain[-1].hash))
    # f.close()
...
def change_difficult():
    if len(blockchain.chain) > 15:
        ...
        if sum_time < time_interval:
            if blockchain.difficulty < 30:
                blockchain.difficulty += 1
        else:
            if blockchain.difficulty > 10:
                blockchain.difficulty -= 1  
...
class Mythread_mine(threading.Thread):
    ...
    def run(self):
        ...
        while True: 
            ...
            # f.close()
            f = open("node 1 log.txt",mode = "a")
            length = len(blockchain.chain[-1].transactions)
            f.write("block{} - len {}".format(len(blockchain.chain),length))
            f.write("\r\n" )
            f.close()
            ...
            for j in blockchain.chain[-1].transactions:
                time_ = blockchain.chain[-1].timestamp - j["timestamp"]
                f.write("tx time:%f   "%j["timestamp"])
                f.write("%f \r\n"%time_)
            f.close()
            ...
            if len(blockchain.chain)>15:
                f = open("average_time.txt",mode="a")
                average_time = 0                
                for i in range(len(blockchain.chain)-10,len(blockchain.chain)):
                    average_time += blockchain.chain[i].timestamp - blockchain.chain[i-1].timestamp
                average_time = average_time / 10
                #f.write("average time:%f %d \r\n"%average_time %difficulty)
                f.write("average time:{} difficulty:{} block length:{}\r\n".format(average_time,blockchain.difficulty,len(blockchain.chain)))
                f.close()
