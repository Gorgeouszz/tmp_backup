...
#                self.DAG.append(tmp)
    ...
    def mine(self,genesis_port):
        ...
        f.write("mine = {} ".format(len(new_block.transactions)))
        f.write("\r\n")
        return True
...
def new_transaction():
    ...
    flag2 = False
    # print("in loop-------------------------")
    # print("length = ",len(blockchain.DAG))
    # print("need index is :",blockchain.DAG[1].DAG[0].hash_)
    # print("real index is :",tx_data['pre1'])
    # print("real index is :",tx_data['pre2'])    
...
def consensus():
    ...
    for node in peers:
        ...
        if length > current_len and blockchain.check_chain_validity(chain):
            ...
            longest_chain = chain
    if longest_chain:
        print("final length = ",len(longest_chain))
    if longest_chain:
        print("befor len = ",len(blockchain.chain))
        blockchain.chain = []
        ...
        for i in range(len(blockchain.DAG)):
            for j in range(3):
                if blockchain.DAG[i].DAG[0] ==tmp_DAG[j][0]:
                    for k in  tmp_DAG[j][0]:
                        blockchain.DAG[i].DAG.append(k)
#把列表里的元素加到现在的DAG中
        interrupt = True
        print("after len = ",len(blockchain.chain))
        for i in blockchain.chain:
            print("transaction num :",len(i.transactions))
        return True
    print("false????????????????????????????????")
    return False
...
class write_log(threading.Thread):
    ...
    def run(self):
        while True:
            time.sleep(5)
            if len(blockchain.chain) > 8 :
                f = open("node 1 log",mode = "w")
                for i in blockchain.chain:
                    for j in i.transactions:
                        f.write("%s   " % j["content"])
                    f.write("\r\n" )
                    # f.write("%s \r" %i.transactions)
                f.close()
                break
